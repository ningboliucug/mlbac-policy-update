...
# 样本数量对应的文件名
sample_sizes = [6, 10, 20, 30, 50, 80, 120]
...
# 准备存储结果的字典
results = {model: {'Mod_Sample_Accuracy': []} for model in styles}
...
for file_path, sample_size in zip(file_paths, sample_sizes):
    ...
    for model in styles:
        if model in ['Retrainer', 'SISA']:
            ...
            avg_mod_sample_acc = model_specific_data['Mod_Sample_Accuracy'].mean() if not model_specific_data.empty else np.nan
            results[model]['Mod_Sample_Accuracy'].append(avg_mod_sample_acc)
        else:
            # 过滤出其他模型的数据
            model_specific_data = data[(data['Model'] == model) & 
                                       (np.abs(data['Test_AUC'] - 0.8) <= 0.01)]
            
            # 找到每一轮中的 Mod_Sample_Accuracy 最大值
            if not model_specific_data.empty:
                max_mod_sample_acc_per_run = model_specific_data.groupby('Run')['Mod_Sample_Accuracy'].max()
                # 计算最大值的均值
                mean_max_mod_sample_acc = max_mod_sample_acc_per_run.mean()
                results[model]['Mod_Sample_Accuracy'].append(mean_max_mod_sample_acc)
            else:
                results[model]['Mod_Sample_Accuracy'].append(np.nan)
...
for model, style in styles.items():
    plt.plot(sample_sizes, results[model]['Mod_Sample_Accuracy'], label=model, **style)

# 设置标题和标签
plt.xlabel('Number of PUTs', fontproperties=font_properties)
plt.ylabel('Accuracy on $Z_{policy}$', fontproperties=font_properties)

# 自定义横坐标，仅显示 10, 30, 50, 80, 120
custom_ticks = [6, 20, 50, 80, 120]
plt.xticks(custom_ticks, [str(size) for size in custom_ticks], fontproperties=font_properties)

# 获取当前的 Axes 对象
ax = plt.gca()
# 手动设置 y 轴的刻度值
yticks = np.linspace(start=0.2, stop=0.9, num=5)  # 设置 5 个刻度
...
# 调整布局并显示图形
output_file_name = f"acc_{model_type}_{data_type}_{mod_type}.png"
