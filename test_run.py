...
from unlearners import Retrainer, First_Order, Fine_Tuning, BFRT, SISA_DNN, SISA_LR
import sys
...
def new_for_ppc(num_runs, num_points, sample_num, n_shards, dataset_path, modification_type, data_type, base_select, model='all'):
    ...
    #retuning_rate_values = linear_values**2 * rr_max
    retuning_rate_values = np.linspace(0, rr_max, num_points)
    bfrt_param_pairs = list(itertools.product(forgetting_rate_values, retuning_rate_values))
    #retuning_rate_values = np.full(num_points, rr_max/2)
    ...
    #for run in range(num_runs):
        ...
        for i in tqdm(range(num_points), desc="Running"):
            ...
            ft_C = ft_C_values[i]
            #forgetting_rate, retuning_rate = bfrt_param_pairs[i]
...
# 全局参数设置
uci_realworld = ('/root/5_MLForAC/dataSet/cleaned_uci-2.0.csv', 'uci')
kaggle_realworld = ('/root/5_MLForAC/dataSet/train.csv', 'kaggle')
dataset = uci_realworld
...
new_for_ppc(num_runs, num_points, sample_num, n_shards, dataset_path, modification_type, data_type, base_select, "all")
#sample_num_list = [120,80,50,30,20,10,6]
